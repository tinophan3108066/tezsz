...
def run(phone, delay, i):
    with concurrent.futures.ThreadPoolExecutor(max_workers=10) as executor:
        ...
        for future in concurrent.futures.as_completed(futures):
            try:
                ...
                error_message = Align.center(
                    f"[bold red on white]LỖI GỌI API: {exc}[/bold red on white]\n"
                    f"[bold green on red]NHƯNG CÁC LUỒNG KHÁC VẪN ĐANG HOẠT ĐỘNG[/bold green on red]",
                    vertical="middle"
                )
                console.print(Panel(error_message, title="[bold red]LỖI[/bold red]", border_style="red"))
    success_panel = Panel(
        f"[bold cyan]✓ SPAM THÀNH CÔNG LẦN {i}! [/bold cyan][blink][/blink]",
    ...
    with Progress(transient=True) as progress:
        task = progress.add_task(
            description="[bold magenta]⏳ ĐANG CHỜ...[/bold magenta]", 
            total=delay
        )
...
def spamcall1(phone, count, delay=1):
...
if __name__ == "__main__":
    ...
    console.print(Align.center(Panel(
        "[bold yellow on blue]🌟 TOOL SPAM SMSCALL [/bold yellow on blue]", 
        subtitle="[italic magenta]Creator: TIENDEV  [/italic magenta]",
        style="bright_blue",
        border_style="yellow"
    )))

    # Nhập thông tin với màu sắc nổi bật
    phone = console.input(
        Align.center("[bold bright_green]📱 SỐ ĐIỆN THOẠI: [/bold bright_green]")
    )
    delay = int(console.input(
        Align.center("[bold bright_cyan]⏲️ THỜI GIAN DELAY (GIÂY): [/bold bright_cyan]")
    ))
    count = int(console.input(
        Align.center("[bold bright_magenta]🔢 SỐ LẦN SPAM: [/bold bright_magenta]")
    ))

    # Thông tin tổng hợp
    info = Columns([
        Panel(f"[bright_green]{phone}[/bright_green]", title="SỐ ĐIỆN THOẠI", style="green", border_style="bright_green"),
        Panel(f"[bright_cyan]{delay}s[/bright_cyan]", title="DELAY", style="cyan", border_style="bright_cyan"),
        Panel(f"[bright_magenta]{count}[/bright_magenta]", title="SỐ LẦN", style="magenta", border_style="bright_magenta"),
    ], align="center")
    console.print(info)

    # Bắt đầu chạy
    console.print(Align.center(Panel(
        "[bold white on red]🚀 KHỞI ĐỘNG SPAM [/bold white on red][blink][/blink]", 
        style="red",
    ...
    # Kết thúc
    console.print(Align.center(Panel(
        "[bold white on green]🎉 HOÀN TẤT SPAM THÀNH CÔNG [/bold white on green]", 
        style="green",
